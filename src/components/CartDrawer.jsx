...
import {
  ...
  closeCart, removeFromCart, increment, decrement, clearCart
...
export default function CartDrawer() {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <aside className={`fixed top-0 right-0 h-full w-full sm:w-[420px] bg-[#111] border-l border-white/10
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/8">
          <div className="flex items-center gap-3">
            ...
            {items.length > 0 && (
              <span className="badge bg-volt/15 text-volt text-xs">{items.length} items</span>
            )}
          </div>
          <button
            onClick={() => dispatch(closeCart())}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center gap-4 text-center py-16">
              ...
              <button onClick={() => dispatch(closeCart())} className="btn-volt mt-2">
                Browse Products
              </button>
            </div>
          ) : (
