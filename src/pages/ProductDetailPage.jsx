...
import {
  ArrowLeft, ShoppingCart, Check, Star, Plus, Minus,
  Truck, Shield, RotateCcw, Heart, Package, ChevronLeft, ChevronRight
} from 'lucide-react'
import { useProduct, useProducts } from '../hooks/useProducts'
import { addToCart, openCart, selectInCart, increment, decrement, selectCartItems } from '../store/cartSlice'
...
function StarRow({ rate }) {
  return (
    <div className="flex items-center gap-1">
      {[...Array(5)].map((_, i) => (
        <Star key={i} size={14}
          className={i < Math.round(rate) ? 'text-amber-400 fill-amber-400' : 'text-white/15 fill-white/15'} />
      ))}
    </div>
  )
}
  ...
  if (isError || !product) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4 text-center">
        ...
        <button onClick={() => navigate('/products')} className="btn-volt">Back to Shop</button>
      </div>
    )
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 xl:gap-16 mb-16">
        ...
        <div className="flex flex-col gap-5 animate-fade-up">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-white/70 text-sm">{product.rating?.rate?.toFixed(1)}</span>
            <span className="text-white/30 text-sm">({product.rating?.count} reviews)</span>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className={`p-3.5 border rounded-2xl transition-all ${wishlisted ? 'bg-red-500/15 border-red-500/30 text-red-400' : 'border-white/10 text-white/30 hover:text-red-400 hover:border-red-500/30'}`}
            >
              <Heart size={20} className={wishlisted ? 'fill-red-400' : ''} />
            </button>
          </div>
