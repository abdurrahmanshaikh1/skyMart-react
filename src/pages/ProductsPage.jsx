...
export default function ProductsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    switch (sort) {
      ...
      case 'rating-desc': list.sort((a, b) => (b.rating?.rate || 0) - (a.rating?.rate || 0)); break
      case 'rating-asc':  list.sort((a, b) => (a.rating?.rate || 0) - (b.rating?.rate || 0)); break
      default: break
    }
  ...
  const clearAll = () => { setSearch(''); setCategory('all'); setSort('default') }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-white/40 font-body text-sm">
          ...
          {category !== 'all' && <span className="text-volt"> in <span className="capitalize">{category}</span></span>}
        </p>
      </div>
          ...
          <div className="flex flex-wrap gap-2 mt-3 pt-3 border-t border-white/6">
            ...
            {sort !== 'default' && (
              <span className="badge bg-volt/10 text-volt border border-volt/20 text-xs gap-1">
                ...
                <button onClick={() => setSort('default')}><X size={10} /></button>
              </span>
            )}
          </div>
        )}
      ...
      {isError && (
        <div className="flex flex-col items-center py-24 gap-4 text-center">
          ...
          <button onClick={() => window.location.reload()} className="btn-volt">Retry</button>
        </div>
      )}
